(() => {
  ...
  const DIM_METRICS=new Set(['latest_season_dim','dim_leader_count']);
  ...
  const PLAYER_GROUPS=[
    ...
    {key:'defense',label:'Backar',items:[
      ['latest_season_dim','DIM i senaste ECL','DIM'],
      ['dim_leader_count','Säsonger som DIM-ledare','GGR']
    ]},
  ...
  function metricValueText(key,val){
    ...
    if(key==='latest_season_dim')return val.toLocaleString('sv-SE',{minimumFractionDigits:2,maximumFractionDigits:2});
  ...
  function matchesMetric(row,key){
    ...
    if(key==='latest_season_dim'||key==='dim_leader_count')return value(row,key)>0;
  ...
  function playerMeta(row,metricKey){
    ...
    if(metricKey==='latest_season_dim'){
      const share=num(row.latest_season_match_share).toLocaleString('sv-SE',{minimumFractionDigits:0,maximumFractionDigits:2});
      return [row.latest_season_label,row.latest_season_team,`${fmt(row.latest_season_games)}/${fmt(row.latest_season_team_games)} matcher`,share?`${share} %`:''].filter(Boolean).join(' · ');
    }
  ...
  function podiumSupportingStats(row,metricKey){
    ...
    if(state.type==='teams'){
      ...
      candidates=[['latest_season_dim','SENASTE ECL'],['dim_leader_count','DIM-LEDARE'],['defender_games','BACK-GP']];
  ...
  function statusSuffix(metricKey){
    if(metricKey==='latest_season_dim')return ' · DIM = (TA + INT + BS) / GP · senaste ECL-säsongen · minst 59 % av lagets matcher';
    if(metricKey==='dim_leader_count')return ' · säsonger/divisioner där spelaren varit bästa svenska back i DIM · minst 59 % av lagets matcher';
