(() => {
  ...
  function teamColorDistance(a, b) {
    return Math.sqrt(
      (a[0] - b[0]) ** 2 +
      (a[1] - b[1]) ** 2 +
      (a[2] - b[2]) ** 2
  ...
  function applyTeamPaletteFromImage(root, image) {
    try {
      if (!root || !(image instanceof HTMLImageElement) || !image.naturalWidth || !image.naturalHeight) return;
      ...
      for (let i = 0; i < pixels.length; i += 4) {
        ...
        // Ignore near-black, near-white and grey logo areas.
        if (light < 28 || light > 232 || saturation < 34) continue;

        const r = Math.min(255, Math.round(r0 / 24) * 24);
        const g = Math.min(255, Math.round(g0 / 24) * 24);
        const b = Math.min(255, Math.round(b0 / 24) * 24);
        const key = r + ',' + g + ',' + b;
        const chroma = Math.max(r, g, b) - Math.min(r, g, b);
        const weight = 1 + chroma / 120;
      ...
      const secondary = colors.find(color => teamColorDistance(color, primary) > 95) || colors[1] || primary;

      root.style.setProperty('--seh-team-accent-rgb', primary.join(','));
      root.style.setProperty('--seh-team-accent-2-rgb', secondary.join(','));
      root.dataset.teamPalette = '1';
    } catch (_) {
      // Some remote legacy logos may be CORS-tainted. Defaults remain in that case.
    }
  }
  ...
  function hydrateTeamLogo(root, primaryUrl, teamName) {
    ...
    for (const shell of shells) {
      ...
      image.addEventListener('load', () => applyTeamPaletteFromImage(root, image));
        ...
        '<div class="seh-dfy__identity">' +
          ...
          '<div class="seh-dfy__identity-copy"><small>DIN SPELARE</small><strong>' + esc(playerName) + '</strong>' +
            (sourceTeamId
              ...
              : '<span class="seh-dfy__clubline seh-dfy__clubline--plain seh-dfy__clubline--free-agent"><span>' + esc(currentTeamText) + '</span></span>') +
          '</div>' +
