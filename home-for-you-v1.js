(() => {
  ...
  function hydrateTeamLogo(root, primaryUrl, teamName) {
    const shell = root?.querySelector?.('.seh-dfy__team-logo');
    const image = shell?.querySelector?.('img');
    if (!shell || !image || !teamName) return;

    if (typeof window.SEH_applyTeamLogo === 'function') {
      try {
        window.SEH_applyTeamLogo(image, primaryUrl ? [primaryUrl] : [], teamName, shell);
        return;
      } catch (_) {}
    }
  ...
  function renderPersonal(host, data) {
      ...
      '<section id="' + ROOT_ID + '" class="seh-desktop-for-you" aria-label="För dig">' +
        ...
        '<div class="seh-dfy__mini-grid">' +
          '<a class="seh-dfy__mini seh-dfy__mini--team" href="' + esc(teamHref) + '">' +
            '<span class="seh-dfy__team-logo"><img alt="" decoding="async"></span>' +
          ...
          '</a>' +
          '<a class="seh-dfy__mini seh-dfy__mini--fa" href="#/free-agents">' +
            '<span class="seh-dfy__mini-copy"><small>FREE AGENTS</small><strong class="seh-dfy__fa-value">' + faCount + '<em>aktiva</em></strong><span>Spelare som söker lag just nu</span></span>' +
            '<b class="seh-dfy__mini-arrow" aria-hidden="true">↗</b>' +
          '</a>' +
        '</div>' +
    ...
    hydrateTeamLogo(host, String(project?.logo_name || '').trim(), currentTeamText);
  }
