(() => {
  ...
  function portraitCard(pos, name, x, y, width, height, team, variant, showStats = false) {
    ...
    const footerHeight = statText ? 72 : 48;
    ...
    return [
      ...
      statText ? '<text x="' + (x+12) + '" y="' + (y+height-43) + '" fill="#ffffff" fill-opacity=".58" font-size="' + statSize + '" font-weight="850" font-family="Arial,Helvetica,sans-serif" letter-spacing=".35">' + esc(statText) + '</text>' : '',
      '<text x="' + (x+12) + '" y="' + (y+height-17) + '" fill="#ffffff" font-size="' + nameSize + '" font-weight="900" font-family="Arial,Helvetica,sans-serif">' + esc(cleanName) + '</text>',
      '</g>'
    ].join("");
  }
