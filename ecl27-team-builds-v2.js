...
(function () {
  ...
  function teamMatchKey(value) {
    return String(value || "")
      ...
      .replace(/\b(?:e[\s-]?sport|esports)\b/g," ")
      .replace(/[^a-z0-9]+/g," ")
      .replace(/\s+/g," ")
      .trim();
  }
  ...
  const baseTeamDefs = [...DATA.springTeams, ...DATA.newTeams].map((team) => ({
    ...team,
  ...
  const teamDefs = (() => {
    ...
    for (const official of DATA.officialSclTeams || []) {
      ...
      if (!target) {
        ...
        defs.push(target);
        byExact.set(norm(target.name),target);
        byMatch.set(teamMatchKey(target.name),target);
      }
      ...
      target.officialRegisteredAt = String(official.registeredAt || "");
      target.officialSyncedAt = String(official.syncedAt || DATA.sclRegistrationUpdated || "");
      target.officialLogoUrl = String(official.logoUrl || "");
    }
    return defs;
  })();
  ...
  function logoUrl(team) {
    ...
    if (team.officialLogoUrl) return String(team.officialLogoUrl);
    return "";
  }
  ...
  function buildModel() {
    ...
    // SportsGamer SCL 27 registration is the highest-priority current roster.
    // Keep the historical timeline, but do not leave stale working-roster
    // players on a team once that team is officially registered.
    for (const team of teams.values()) {
      if (!team.officialScl) continue;

      const desired = new Map();
      for (const rawPlayer of team.officialRoster || []) {
        const player = canonical(rawPlayer);
        const key = playerKey(player);
        if (key) desired.set(key,player);
      }
  ...
  window.SEH_ECL27_CURRENT_ROSTER_V1 = {
    ...
    teams: model.teams.map((team) => ({
      ...
      officialScl:Boolean(team.officialScl),
      officialName:team.officialName || '',
      sportsGamerTeamId:team.sportsGamerTeamId || null,
      sportsGamerLeagueId:team.sportsGamerLeagueId || null,
  ...
  function recruitmentNeeds(team) {
    ...
    if (/\b(målvakt|malvakt|goalie|keeper)\b/i.test(text)) add("G");
    if (/\b(vänsterback|vansterback|left\s*defen[cs]e|\bld\b)\b/i.test(text)) add("LD");
    if (/\b(högerback|hogerback|right\s*defen[cs]e|\brd\b)\b/i.test(text)) add("RD");
    if (!needs.includes("LD") && !needs.includes("RD") &&
        /\b(back|backar|defender|defenders|defence|defense)\b/i.test(text)) add("D");
    if (/\b(center|centre|\bc\b)\b/i.test(text)) add("C");
    if (/\b(vänsterforward|vansterforward|left\s*wing|\blw\b)\b/i.test(text)) add("LW");
    if (/\b(högerforward|hogerforward|right\s*wing|\brw\b)\b/i.test(text)) add("RW");
    if (!needs.some((role) => ["C","LW","RW"].includes(role)) &&
        /\b(forward|forwards|fw)\b/i.test(text)) add("F");
  ...
  function statusFor(team) {
    ...
    const activeRecruitment = Boolean(team.recruitment);
    const needs = recruitmentNeeds(team);

    if (activeRecruitment) {
      const needLabel = needs.length ? ` · söker ${needs.join(" + ")}` : " · söker spelare";
      const tone = count >= 5 ? "yellow" : count >= 3 ? "orange" : "red";
      return {
        key: count >= 5 ? "building" : count >= 3 ? "thin" : "rebuild",
        label: `Ej färdig${needLabel}`,
        tone
      };
    }
  ...
  function sportsGamerTeamUrl(team) {
    const leagueId = Number(team?.sportsGamerLeagueId) || Number(DATA.sclLeagueId) || 527;
    const teamId = Number(team?.sportsGamerTeamId);
    return teamId > 0 ? `https://sportsgamer.gg/leagues/${leagueId}/teams/${teamId}` : "";
  }

  function officialSclBadge(team) {
    return team?.officialScl ? '<b class="is-official">OFFICIELLT SCL-LAG</b>' : "";
  }
  ...
  function renderTeamHeader(team,logo,status) {
    ...
    return `<header>${logoMarkup}<div><p>${esc(source)}</p>${nameMarkup}<div class="ecl27v2-badges"><span>${team.kind === "new" ? "NYTT" : esc(team.division)}</span>${officialSclBadge(team)}<b class="is-${status.tone}">${esc(status.label)}</b></div></div></header>`;
  }
  ...
  function renderTeamCard(team) {
    ...
    return `<article class="ecl27v2-card is-clickable" data-status="${status.key}" data-team="${esc(team.name)}" data-build-url="${esc(buildUrl)}" role="link" tabindex="0" aria-label="Öppna ECL 27-lagbygget för ${esc(team.name)}">
      ...
      <div class="ecl27v2-metrics">
        <div><span>${team.officialScl ? "REG. SCL" : "KÄNDA NU"}</span><strong>${team.playersNow.length}</strong></div>
      ...
      <section class="ecl27v2-roster"><label>${team.officialScl ? "OFFICIELL SCL-TRUPP" : "KÄND TRUPP JUST NU"}</label><div>${team.playersNow.length ? team.playersNow.map((name) => playerLink(name,"ecl27v2-roster-player")).join("") : `<em>${team.officialScl ? "Inga spelare registrerade ännu." : "Ingen säker spelare kvar i sammanställningen."}</em>`}</div></section>
  ...
  function filteredTeams() {
    return sortedTeams().filter((team) => {
      ...
      const haystack = [
        team.name,team.springName,team.officialName,...team.playersNow,
  ...
  function normalizeCountryCode(value) {
    ...
    const aliases = {
      ...
      CZE:"CZ", SVK:"SK", POL:"PL", AUT:"AT", SUI:"CH",
      GBR:"GB", USA:"US", CAN:"CA", LAT:"LV", LET:"LV"
    };
    return aliases[code] || code;
  }

  function countryFlagMarkup(value) {
    const code = normalizeCountryCode(value);
    if (!/^[A-Z]{2}$/.test(code)) return "";

    let dataUri = "";
    if (typeof window.SEH_countryFlagDataUri === "function") {
      dataUri = window.SEH_countryFlagDataUri(code);
    } else if (typeof SEH_countryFlagDataUri === "function") {
      dataUri = SEH_countryFlagDataUri(code);
    }

    if (!dataUri) return "";
    return `<i class="seh-country-flag ecl27v2-country-flag" role="img" aria-label="${esc(code)}" title="${esc(code)}" style="background-image:url('${esc(dataUri)}')"></i>`;
  }
  ...
  function renderDetailRosterCard(team, name, row=null) {
    ...
    const countryCode = normalizeCountryCode(row?.player_country);
    const flag = countryFlagMarkup(countryCode);
    ...
    return `<a class="ecl27v2-roster-feature${row ? " is-hydrated" : " is-loading"}" href="${esc(playerUrl(name))}" data-player-card-key="${esc(playerKey(name))}">
      ...
      <div class="ecl27v2-roster-feature__body">
        <div class="ecl27v2-roster-feature__top"><span>AKTUELL TRUPP</span><div class="ecl27v2-roster-feature__top-meta">${flag}<b>${esc(position)}</b></div></div>
        <h4>${esc(name)}</h4>
  ...
  async function fetchDirectoryRows(names) {
    ...
    const select = "player_key,display_gamertag,player_country,player_image,sports_gamer_player_url,primary_position,latest_season,latest_team,career_games,total_points,player_type,tournament_count,total_goalie_saves";
    ...
    if (missing.length) {
      ...
      if (fallbackError) {
        ...
        const fallbackRows = Array.isArray(fallback) ? fallback.map((row) => ({...row})) : [];
        const fallbackNames = fallbackRows
          .map((row) => String(row?.display_gamertag || "").trim())
          .filter(Boolean);

        if (fallbackNames.length) {
          const {data:countryRows,error:countryError} = await client
            .from("ehockey_player_history_cache_v25")
            .select("display_gamertag,player_country,chronology_date")
            .in("display_gamertag",fallbackNames)
            .order("chronology_date",{ascending:false});

          if (countryError) {
            console.warn("[ECL27] kunde inte hämta nationalitet för fallback-spelare",countryError);
          } else {
            const countryByName = new Map();
            for (const countryRow of countryRows || []) {
              const key = norm(countryRow.display_gamertag);
              if (key && !countryByName.has(key) && countryRow.player_country) {
                countryByName.set(key,countryRow.player_country);
              }
            }
            for (const row of fallbackRows) {
              row.player_country = row.player_country || countryByName.get(norm(row.display_gamertag)) || "";
            }
          }
        }
  ...
  function renderTeamDetail(team) {
    ...
    const source = team.officialScl
      ...
      : team.kind === "new"
        ...
        : `ECL 26 SPRING · ${team.division}${team.springName && team.springName !== team.name ? ` · ${team.springName}` : ""}`;
    const sportsGamerUrl = sportsGamerTeamUrl(team);
    ...
    return `<div class="ecl27v2-detail">
      ...
      <header class="ecl27v2-detail-hero">
        ...
        <div class="ecl27v2-detail-title">
          ...
          <h2>${esc(team.name)}</h2>
          <div class="ecl27v2-badges"><span>${team.kind === "new" ? "NYTT" : esc(team.division)}</span>${officialSclBadge(team)}<b class="is-${status.tone}">${esc(status.label)}</b></div>
          <p>${team.officialScl
            ? "Laget är officiellt anmält till SCL 27. Den registrerade SportsGamer-truppen används som högsta prioritet för aktuell trupp, medan tidigare IN/UT-historik ligger kvar som kronologi."
            : "Det här är Svensk eHockeys aktuella arbetsbild av lagbygget inför ECL 27, baserad på Spring-truppen och de daterade IN/UT-, lagpost- och ECL-Free Agent-händelser som finns i underlaget."}</p>
          ${regularUrl ? `<a class="ecl27v2-team-profile" href="${esc(regularUrl)}">Öppna ordinarie lagprofil →</a>` : ""}
          ${sportsGamerUrl ? `<a class="ecl27v2-team-profile" href="${esc(sportsGamerUrl)}" target="_blank" rel="noopener">Öppna SCL-registreringen ↗</a>` : ""}
        </div>
      </header>
      ...
      <section class="ecl27v2-detail-panel ecl27v2-detail-roster-featured">
        <div class="ecl27v2-detail-panel-head"><div><p class="directory-kicker">JUST NU</p><h3>${team.officialScl ? "Officiell SCL-trupp" : "Känd trupp"}</h3></div><div class="ecl27v2-roster-status"><b class="is-${status.tone}">${esc(status.label)}</b><span>${team.playersNow.length} spelare</span></div></div>
        <div id="ecl27v2DetailRosterCards" class="ecl27v2-roster-feature-grid">${team.playersNow.length
          ...
          : `<p class="ecl27v2-detail-empty">${team.officialScl ? "Inga spelare är registrerade på SCL-truppen ännu." : "Ingen säker spelare kvar i sammanställningen."}</p>`}</div>
      </section>
      ...
      <aside class="ecl27v2-method"><strong>${team.officialScl ? "Officiell SCL-registrering + lagbyggehistorik" : "Arbetsbild – inte officiell ECL 27-roster"}</strong>${team.officialScl ? "SportsGamer SCL 27-truppen skriver över arbetsbilden för aktuell trupp. Transferhistoriken visas fortfarande för sammanhang." : "En UT-händelse kan inte lämna samma spelare kvar i KÄND TRUPP JUST NU. En senare IN flyttar spelaren till det nya laget. ECL-Free Agents behandlas som UT från senast kända ECL-lag."}</aside>
    </div>`;
  }
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .ecl27v2-badges{display:flex;flex-wrap:wrap;gap:6px;margin-top:8px}.ecl27v2-badges span,.ecl27v2-badges b{padding:4px 7px;border:1px solid #264056;border-radius:999px;font-size:8px;letter-spacing:.06em}.ecl27v2-badges .is-green{color:#44e6b9}.ecl27v2-badges .is-yellow{color:#f0d58b}.ecl27v2-badges .is-orange{color:#ffb35f}.ecl27v2-badges .is-red{color:#ff7e7e}.ecl27v2-badges .is-official{border-color:rgba(87,230,220,.55);background:rgba(87,230,220,.08);color:#57e6dc;font-weight:950}
      ...
      .ecl27v2-hero>div>p:last-of-type{
        max-width:760px;
        margin:0;
        color:#a2a8a5;
        font-size:.96rem;
      ...
      .ecl27v2-overview>div{
        min-height:108px;
        padding:20px;
        border-right-color:rgba(255,255,255,.12);
      }
      .ecl27v2-overview span{color:#858f8c}
      .ecl27v2-overview strong{
        color:#ffd000;
        font-family:"Arial Black",Impact,Haettenschweiler,sans-serif;
        font-size:2.15rem;
      }
      .ecl27v2-overview small{color:#707875}
      .ecl27v2-panel{
        margin-top:22px;
        border-color:rgba(255,255,255,.12);
        border-radius:0;
      ...
      .ecl27v2-toolbar label{
        color:#79ddd6;
      }
      .ecl27v2-toolbar input,
      .ecl27v2-toolbar select{
        height:46px;
        border-color:rgba(255,208,0,.28);
        border-radius:0;
        background:#101111;
      }
      .ecl27v2-toolbar input:focus,
      .ecl27v2-toolbar select:focus{
        outline:none;
        border-color:#ffd000;
        box-shadow:inset 0 0 0 1px rgba(255,208,0,.16);
      }
      .ecl27v2-grid{
        gap:0;
        padding:0;
      ...
      .ecl27v2-card.is-clickable:hover,
      .ecl27v2-card.is-clickable:focus-visible{
        transform:none;
        border-color:rgba(255,255,255,.12);
        background:linear-gradient(145deg,rgba(255,208,0,.045),#0b0d0d 52%);
        box-shadow:none;
      }
      .ecl27v2-card-open{
        color:#ffd000;
      ...
      .ecl27v2-badges b,
      .ecl27v2-metrics>div,
      .ecl27v2-roster-player,
      .ecl27v2-recruit,
      .ecl27v2-back,
      .ecl27v2-detail-logo,
      .ecl27v2-detail-metrics>div,
      .ecl27v2-detail-panel,
      .ecl27v2-team-profile,
      .ecl27v2-roster-status b,
      .ecl27v2-detail-base-player{
        border-radius:0;
      }
      .ecl27v2-logo,
      .ecl27v2-detail-logo{
      ...
      .ecl27v2-card header p,
      .ecl27v2-roster label,
      .ecl27v2-recruit label,
      .ecl27v2-moves label{
        color:#7adfd8;
      }
      .ecl27v2-card h3{
      ...
      .ecl27v2-detail-title h2{
        font-family:"Arial Black",Impact,Haettenschweiler,sans-serif;
        letter-spacing:-.06em;
      }
      .ecl27v2-detail-metrics{
        gap:0;
        margin-top:22px;
        border-top:1px solid rgba(255,255,255,.12);
        border-left:1px solid rgba(255,255,255,.12);
      ...
      @media(max-width:780px){
        ...
        .ecl27v2-stamp{min-height:150px}
        .ecl27v2-overview{
        ...
        .ecl27v2-head h3{font-size:1.65rem}
        .ecl27v2-toolbar{padding:12px}
        .ecl27v2-card{padding:16px}
        .ecl27v2-detail-hero{
        ...
        .ecl27v2-detail-title h2{font-size:2.4rem}
        .ecl27v2-detail-panel{padding:16px}
      }
  ...
  function renderListSection(section) {
    ...
    const officialCount = model.teams.filter((team) => team.officialScl).length;
