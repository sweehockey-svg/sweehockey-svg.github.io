(() => {
  ...
  async function ensureAdmin() {
    ...
    const { data, error } = await sb.rpc("seh_fantasy_admin_state", { p_code: "SCL25BETA" });
  ...
  async function loadSyncState() {
    const { data, error } = await sb.rpc("seh_fantasy_admin_sync_state", { p_code: "SCL25BETA" });
  ...
  async function recalculateEntries() {
    ...
    try {
      const { data, error } = await sb.rpc("seh_fantasy_admin_recalculate", { p_code: "SCL25BETA" });
      if (error) throw error;
      await loadAll();
