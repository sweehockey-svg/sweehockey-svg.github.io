(() => {
  ...
  const sb = window.supabase.createClient(supabaseUrl, supabaseKey, {
    auth: {
      storageKey: "seh-scl25-fantasy-auth",
      persistSession: true,
  ...
  function showBuilder() {
    ...
    if (!hasPool) {
      setStatus("saveStatus", "Spelarpoolen öppnas när SCL 25-rostrarna och priserna är klara.");
    } else if (!open) {
  ...
  async function loadPublic() {
    ...
    if (!state.competition) throw new Error("SCL 25 Fantasy är inte konfigurerad.");
  ...
  (async () => {
    try {
      ...
      console.error("SCL 25 Fantasy kunde inte laddas", error);
