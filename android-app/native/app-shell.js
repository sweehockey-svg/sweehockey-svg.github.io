(function () {
  ...
  function ensureBottom(){
    ...
    nav=document.createElement('nav');nav.id='seh-native-bottom';nav.innerHTML=`<a data-tab="news" href="#/nyheter">${icons.news}<span>Nyheter</span></a><button data-tab="directory" id="seh-directory-btn">${icons.players}<span>Spelare & Lag</span></button><a data-tab="sec" href="${ROOT}SEC/">${icons.sec}<span>SEC</span></a><a data-tab="shop" href="#/shop">${icons.shop}<span>Shop</span></a><button data-tab="more" id="seh-more-btn">${icons.more}<span>Mer</span></button>`;document.body.appendChild(nav);
  ...
  function ensureDirectory(){
    ...
    dir.innerHTML=`<div class="seh-app-page"><div class="seh-kicker">SVENSK eHOCKEY</div><h1>Spelare & Lag</h1><p class="lead">Välj vad du vill utforska.</p><div class="seh-card-grid"><a class="seh-card wide" href="#/spelare" data-load><div><div class="ico">${icons.players}</div><strong>Spelare</strong><span>Sök profiler, historik och statistik.</span></div></a><a class="seh-card wide gold" href="#/laghistoria" data-load><div><div class="ico">${icons.teams}</div><strong>Lag</strong><span>Svenska lag, klubbhistorik och tidigare spelartrupper.</span></div></a></div></div>`;
  ...
  function refreshBottom(){const r=route();const directoryOpen=document.getElementById('seh-app-directory')?.classList.contains('show');const moreOpen=document.getElementById('seh-app-more')?.classList.contains('show')||document.getElementById('seh-app-favorites')?.classList.contains('show');document.querySelectorAll('#seh-native-bottom [data-tab]').forEach(a=>a.classList.toggle('on',directoryOpen?a.dataset.tab==='directory':moreOpen?a.dataset.tab==='more':a.dataset.tab===r.tab));}
