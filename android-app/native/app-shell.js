(function () {
  ...
  const CSS = `
    ...
    #seh-app-home,#seh-app-more,#seh-app-favorites{position:fixed;z-index:2147481000;left:0;right:0;top:var(--seh-native-top);bottom:calc(var(--seh-native-bottom) + env(safe-area-inset-bottom));background:var(--seh-bg);overflow-y:auto;overscroll-behavior-y:contain;color:var(--seh-text);display:none}
    #seh-app-home.show,#seh-app-more.show,#seh-app-favorites.show{display:block}
    ...
    @media(max-width:390px){.seh-card-grid{gap:9px}.seh-card{padding:14px;min-height:125px}.seh-app-page{padding-left:14px;padding-right:14px}#seh-native-top .logo img{width:58px}}
  `;

  function htmlEscape(s){return String(s||'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));}
  function route(){
    if(isSec()) return {tab:'more',title:'SEC',kind:'sec'};
    ...
    if(h.startsWith('#/laghistoria')||h.startsWith('#/lag/')) return {tab:'more',title:h.startsWith('#/lag/')?'Lag':'Laghistoria',kind:h.startsWith('#/lag/')?'team':'teams'};
  ...
  function ensureTop(){
    ...
    top.querySelector('#seh-back').onclick=()=>{if(document.getElementById('seh-app-more')?.classList.contains('show')||document.getElementById('seh-app-favorites')?.classList.contains('show')){closeOverlays();return;}history.back();};
  ...
  function ensureBottom(){
    ...
    nav=document.createElement('nav');nav.id='seh-native-bottom';nav.innerHTML=`<a data-tab="home" href="#/">${icons.home}<span>Hem</span></a><a data-tab="news" href="#/nyheter">${icons.news}<span>Nyheter</span></a><a data-tab="players" href="#/spelare">${icons.players}<span>Spelare</span></a><a data-tab="shop" href="#/shop">${icons.shop}<span>Shop</span></a><button data-tab="more" id="seh-more-btn">${icons.more}<span>Mer</span></button>`;document.body.appendChild(nav);
  ...
  function ensureHome(){
    ...
    home=document.createElement('section');home.id='seh-app-home';home.innerHTML=`<div class="seh-app-page"><div class="seh-kicker">SVENSK eHOCKEY / APP</div><h1>All svensk eHockey.<br>En app.</h1><p class="lead">Snabbvägar till det viktigaste på Svensk eHockey – nyheter, spelare, lag, SEC och shoppen.</p><div class="seh-card-grid"><a class="seh-card wide gold" href="#/nyheter" data-load><div><div class="ico">${icons.news}</div><strong>Senaste nytt</strong><span>Artiklar, uppdateringar och det senaste från svensk eHockey.</span></div></a><a class="seh-card" href="#/spelare" data-load><div><div class="ico">${icons.players}</div><strong>Spelare</strong><span>Sök profiler, historik och statistik.</span></div></a><a class="seh-card" href="#/laghistoria" data-load><div><div class="ico">${icons.teams}</div><strong>Svenska lag</strong><span>Klubbar, historik och tidigare spelartrupper.</span></div></a><a class="seh-card gold" href="${ROOT}SEC/" data-load><div><div class="ico">${icons.sec}</div><strong>SEC</strong><span>Svenska eHockey Cupen – turneringar och statistik.</span></div></a><a class="seh-card" href="#/shop" data-load><div><div class="ico">${icons.shop}</div><strong>Shop</strong><span>Svensk eHockey-design och personliga lagprodukter.</span></div></a><div class="seh-card wide disabled" id="seh-ecl-card"><span class="seh-badge">SNART</span><div><div class="ico">${icons.ecl}</div><strong>ECL-statistik</strong><span>Appplatsen är klar. Funktionen kopplas på när den gömda ECL-statistiken är redo.</span></div></div></div><div class="seh-section-title"><h2>Din app</h2><small id="seh-fav-count"></small></div><div class="seh-list"><button class="seh-list-item" id="seh-home-favs"><span class="bullet">${icons.heart}</span><span><strong>Favoriter</strong><span>Spelare, lag och artiklar du sparat.</span></span></button><button class="seh-list-item" id="seh-home-more"><span class="bullet">${icons.more}</span><span><strong>Mer</strong><span>SEC, lag, notiser, favoriter och appinställningar.</span></span></button></div></div>`;
  ...
  function closeOverlays(){document.getElementById('seh-app-more')?.classList.remove('show');document.getElementById('seh-app-favorites')?.classList.remove('show');refreshTop();refreshBottom();}
  function openMore(){ensureMore();document.getElementById('seh-app-favorites')?.classList.remove('show');document.getElementById('seh-app-more').classList.add('show');renderNotifyToggle();refreshTop();refreshBottom();}
  function openFavorites(){ensureFavorites();document.getElementById('seh-app-more')?.classList.remove('show');document.getElementById('seh-app-favorites').classList.add('show');renderFavorites();refreshTop();refreshBottom();}
  ...
  function refreshTop(){
    const r=route();const title=document.getElementById('seh-native-title');if(title)title.textContent=document.getElementById('seh-app-more')?.classList.contains('show')?'Mer':document.getElementById('seh-app-favorites')?.classList.contains('show')?'Favoriter':r.title;
    const back=document.getElementById('seh-back');if(back)back.style.visibility=(r.kind==='home'&&!document.getElementById('seh-app-more')?.classList.contains('show')&&!document.getElementById('seh-app-favorites')?.classList.contains('show'))?'hidden':'visible';
  ...
  function refreshBottom(){const r=route();const moreOpen=document.getElementById('seh-app-more')?.classList.contains('show')||document.getElementById('seh-app-favorites')?.classList.contains('show');document.querySelectorAll('#seh-native-bottom [data-tab]').forEach(a=>a.classList.toggle('on',moreOpen?a.dataset.tab==='more':a.dataset.tab===r.tab));}
  function refreshHome(){const home=document.getElementById('seh-app-home');if(home)home.classList.toggle('show',route().kind==='home'&&!document.getElementById('seh-app-more')?.classList.contains('show')&&!document.getElementById('seh-app-favorites')?.classList.contains('show'));updateFavCount();}
  ...
  function ensureAll(){unlockScrolling();ensureStyle();ensureTop();ensureBottom();ensureHome();ensureMore();ensureFavorites();ensureLoader();ensurePull();ensureOffline();installPullToRefresh();interceptLinks();}
  ...
  let lastRoute='';function closeOverlaysIfRouteChanged(){const now=location.pathname+location.hash;if(lastRoute&&lastRoute!==now){document.getElementById('seh-app-more')?.classList.remove('show');document.getElementById('seh-app-favorites')?.classList.remove('show');}lastRoute=now;}
