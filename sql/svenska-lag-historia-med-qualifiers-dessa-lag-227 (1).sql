...
WITH
/* =========================================================
   SVENSKA LAG HISTORIA - ALL ECL - V10
   Output: one JSON object with all Swedish ECL team seasons,
   players, player stats and player/team history.

   Swedish team = nhlgamer_leagueTeams.country = 'SE' OR manually listed Swedish teamID (227 IDs). Team name is taken from nhlgamer_leagueTeams.teamName per league season, so same teamID can show different names in different leagues.
...
ecl_league_map_raw AS (
  ...
  UNION ALL SELECT 64, 'ELITE QUALIFIER', NULL, NULL, NULL
  ...
  UNION ALL SELECT 115, 'PRO QUALIFIER', NULL, NULL, NULL
  UNION ALL SELECT 116, 'PRO QUALIFIER', NULL, NULL, NULL
  UNION ALL SELECT 117, 'LITE QUALIFIER', NULL, NULL, NULL
  ...
  UNION ALL SELECT 166, 'PRO QUALIFIER', NULL, 2022, 'Winter'
  UNION ALL SELECT 167, 'PRO QUALIFIER', NULL, 2022, 'Winter'
  UNION ALL SELECT 168, 'LITE QUALIFIER', NULL, 2022, 'Winter'
  UNION ALL SELECT 169, 'LITE QUALIFIER', NULL, 2022, 'Winter'
  ...
  UNION ALL SELECT 176, 'ELITE QUALIFIER', NULL, 2022, 'Winter'
  UNION ALL SELECT 183, 'PRO QUALIFIER', NULL, 2022, 'Spring'
  UNION ALL SELECT 184, 'PRO QUALIFIER', NULL, 2022, 'Spring'
  UNION ALL SELECT 186, 'LITE QUALIFIER', NULL, 2022, 'Spring'
  UNION ALL SELECT 187, 'LITE QUALIFIER', NULL, 2022, 'Spring'
  ...
  UNION ALL SELECT 243, 'PRO QUALIFIER', NULL, 2023, 'Winter'
  UNION ALL SELECT 244, 'PRO QUALIFIER', NULL, 2023, 'Winter'
  UNION ALL SELECT 246, 'LITE QUALIFIER', NULL, 2023, 'Winter'
  UNION ALL SELECT 247, 'LITE QUALIFIER', NULL, 2023, 'Winter'
  ...
  UNION ALL SELECT 262, 'ELITE QUALIFIER', NULL, 2023, 'Winter'
  UNION ALL SELECT 295, 'ELITE QUALIFIER', NULL, 2023, 'Spring'
  UNION ALL SELECT 296, 'PRO QUALIFIER', NULL, 2023, 'Spring'
  UNION ALL SELECT 297, 'PRO QUALIFIER', NULL, 2023, 'Spring'
  UNION ALL SELECT 299, 'LITE QUALIFIER', NULL, 2023, 'Spring'
  ...
  UNION ALL SELECT 328, 'ELITE QUALIFIER', NULL, 2024, 'Winter'
  UNION ALL SELECT 329, 'PRO QUALIFIER', NULL, 2024, 'Winter'
  UNION ALL SELECT 332, 'LITE QUALIFIER', NULL, 2024, 'Winter'
  ...
  UNION ALL SELECT 375, 'ELITE QUALIFIER', NULL, 2024, 'Spring'
  UNION ALL SELECT 376, 'PRO QUALIFIER', NULL, 2024, 'Spring'
  UNION ALL SELECT 377, 'PRO QUALIFIER WILDCARD', NULL, 2024, 'Spring'
  UNION ALL SELECT 378, 'LITE QUALIFIER', NULL, 2024, 'Spring'
  ...
  UNION ALL SELECT 403, 'ELITE QUALIFIER', NULL, 2025, 'Winter'
  UNION ALL SELECT 404, 'PRO QUALIFIER', NULL, 2025, 'Winter'
  UNION ALL SELECT 405, 'PRO QUALIFIER', NULL, 2025, 'Winter'
  UNION ALL SELECT 406, 'LITE QUALIFIER', NULL, 2025, 'Winter'
  ...
  UNION ALL SELECT 456, 'ELITE QUALIFIER', NULL, 2025, 'Spring'
  UNION ALL SELECT 458, 'PRO QUALIFIER', NULL, 2025, 'Spring'
  UNION ALL SELECT 459, 'PRO QUALIFIER', NULL, 2025, 'Spring'
  UNION ALL SELECT 460, 'PRO QUALIFIER', NULL, 2025, 'Spring'
  ...
  UNION ALL SELECT 484, 'PRO QUALIFIER', NULL, 2026, 'Winter'
  UNION ALL SELECT 485, 'PRO QUALIFIER', NULL, 2026, 'Winter'
  UNION ALL SELECT 486, 'PRO QUALIFIER', NULL, 2026, 'Winter'
...
ecl_league_map AS (
  SELECT
    ...
    CASE
      ...
      WHEN leagueID IN (64,65,66,67,68) THEN 'ECL 10'
      WHEN leagueID IN (94,95,96,97) THEN 'ECL 11'
      WHEN leagueID IN (115,116,117,119,120,121,122,123) THEN 'ECL 12'
      WHEN leagueID IN (166,167,168,169,170,171,172,173,174,176) THEN 'ECL 22 Winter'
      WHEN leagueID IN (183,184,186,187,190,191,192,193,194) THEN 'ECL 22 Spring'
      WHEN leagueID IN (243,244,246,247,250,251,252,253,254,262) THEN 'ECL 23 Winter'
      WHEN leagueID IN (295,296,297,299,305,306,307,308,309) THEN 'ECL 23 Spring'
      WHEN leagueID IN (328,329,332,338,339,340,341,342) THEN 'ECL 24 Winter'
      WHEN leagueID IN (375,376,377,378,379,380,381,382,383) THEN 'ECL 24 Spring'
      WHEN leagueID IN (403,404,405,406,411,412,413,414,415) THEN 'ECL 25 Winter'
      WHEN leagueID IN (456,458,459,460,461,462,463,464,465) THEN 'ECL 25 Spring'
...
SELECT JSON_OBJECT(
  ...
  'source', 'svenska-lag-historia-cards-v9-league-team-names.sql',
  'summary', JSON_OBJECT(
    'teamSeasonCount', (SELECT COUNT(*) FROM swedish_team_seasons),
    'uniqueTeamCount', (SELECT COUNT(DISTINCT teamID) FROM swedish_team_seasons),
    'uniquePlayerCount', (
      SELECT COUNT(DISTINCT pts.playerID)
      FROM player_team_stats pts
      JOIN swedish_team_seasons sts
        ON sts.leagueID = pts.leagueID
       AND sts.teamID = pts.teamID
    ),
    'totalGames', (
      SELECT COALESCE(SUM(ts.gamesPlayed), 0)
      FROM team_stats ts
      JOIN swedish_team_seasons sts
        ON sts.leagueID = ts.leagueID
       AND sts.teamID = ts.teamID
    ),
    'totalWins', (
      SELECT COALESCE(SUM(ts.wins), 0)
      FROM team_stats ts
      JOIN swedish_team_seasons sts
        ON sts.leagueID = ts.leagueID
       AND sts.teamID = ts.teamID
    ...
    'totalLosses', (
      SELECT COALESCE(SUM(ts.losses), 0)
      FROM team_stats ts
      JOIN swedish_team_seasons sts
        ON sts.leagueID = ts.leagueID
       AND sts.teamID = ts.teamID
  ...
  'teams', COALESCE((
    SELECT JSON_ARRAYAGG(
      JSON_OBJECT(
        ...
        'teamID', q.teamID,
        'name', q.teamName,
        'seasonTeamName', q.teamName,
        'currentName', COALESCE(q.currentTeamName, q.originalTeamName, q.teamName),
        'mappedName', COALESCE(q.mappedTeamName, q.originalTeamName, q.teamName),
        'originalName', q.originalTeamName,
        'country', q.teamCountry,
        'groupID', q.groupID,
        'division', q.divisionName,
        ...
        'playerCount', COALESCE(q.playerCount, 0),
        'playedPlayerCount', COALESCE(q.playedPlayerCount, 0),
        'topPlayerName', COALESCE(q.topPlayerName, ''),
        'topPlayerPoints', COALESCE(q.topPlayerPoints, 0)
      )
    )
    FROM (
      SELECT
        ...
        COUNT(DISTINCT CASE WHEN pts.played THEN pts.playerID END) AS playedPlayerCount,
        SUBSTRING_INDEX(
          GROUP_CONCAT(pts.playerName ORDER BY pts.points DESC, pts.gamesPlayed DESC, pts.playerName ASC SEPARATOR '||'),
          '||',
          1
        ) AS topPlayerName,
        MAX(pts.points) AS topPlayerPoints
      FROM swedish_team_seasons sts
      LEFT JOIN team_stats ts
        ON ts.leagueID = sts.leagueID
       AND ts.teamID = sts.teamID
      LEFT JOIN player_team_stats pts
        ON pts.leagueID = sts.leagueID
       AND pts.teamID = sts.teamID
      GROUP BY
        sts.leagueID,
        sts.teamID,
        sts.teamName,
        sts.currentTeamName,
        sts.mappedTeamName,
        sts.originalTeamName,
        sts.teamCountry,
        sts.groupID,
        sts.registeredForLeague,
        sts.teamCaptainID,
        sts.teamAssistantCaptainID,
        sts.teamAssistantCaptainID2,
        sts.divisionName,
        sts.divisionKey,
        sts.divisionRank,
        sts.seasonYear,
