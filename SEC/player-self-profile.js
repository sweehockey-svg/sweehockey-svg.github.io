...
(function () {
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      .secSelfSocials{display:flex;flex-wrap:wrap;gap:8px;margin-top:12px}
      .secSelfSocials a{display:inline-flex;align-items:center;gap:7px;min-height:32px;padding:7px 11px;border:1px solid rgba(205,169,83,.35);border-radius:999px;background:rgba(3,10,17,.76);color:#edf2f5;text-decoration:none;font-size:12px;font-weight:800;transition:border-color .15s ease,background .15s ease,color .15s ease}
      ...
      @media(max-width:760px){.secSelfProfileFacts{grid-template-columns:1fr}.secSelfProfileBox{padding:13px}.secSelfProfileBoxHead{align-items:flex-start}.secSelfSocials{gap:6px}.secSelfSocials a{padding:6px 9px;font-size:11px}}
    `;
  ...
  function socialLinks(row) {
    return [
      { label: "X", url: text(row?.x_url), mark: "𝕏" },
      { label: "Twitch", url: text(row?.twitch_url), mark: "TV" },
      { label: "Instagram", url: text(row?.instagram_url), mark: "IG" }
  ...
  function applyProfileRow(row) {
    ...
    if (links.length) {
      ...
      socials.innerHTML = links.map(function (item) {
        return `<a href="${escapeAttribute(item.url)}" target="_blank" rel="noopener noreferrer"><span class="secSelfSocialsMark">${escapeHtml(item.mark)}</span>${escapeHtml(item.label)}</a>`;
      }).join("");
