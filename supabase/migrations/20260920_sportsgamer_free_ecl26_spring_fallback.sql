...
chosen_ecl as (
  (
    select
      team_name,division,season_short,true as is_ecl26_spring
    from ecl_rows
    where is_ecl26_spring
    order by chronology_date desc nulls last,
             sports_gamer_league_id desc,
             games desc
    limit 1
  )
  union all
  (
    select
      team_name,division,season_short,false as is_ecl26_spring
    from ecl_rows
