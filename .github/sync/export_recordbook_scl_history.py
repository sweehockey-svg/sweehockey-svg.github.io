#!/usr/bin/env python3
"""Export official SCL match and goal-event data from SportsGamer, read-only."""
...
SYNC_VERSION = "scl-recordbook-v1.1"
...
def league_ids() -> list[int]:
    ...
    if not result or len(result) > 10:
        raise RuntimeError("Expected 1-10 SportsGamer league IDs")
    return result
...
def main() -> int:
    ...
    if len(match_rows) < 100:
        raise RuntimeError(f"Safety stop: only {len(match_rows)} SCL matches exported")
    if len(goal_rows) < 100:
        raise RuntimeError(f"Safety stop: only {len(goal_rows)} official SCL goal events exported")
