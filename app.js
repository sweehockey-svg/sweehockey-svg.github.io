...
function SEH_initHistory() {
  ...
  (() => {
    ...
    const APP_BUILD = "2026-09-11-v13010-team-rp-history";
    ...
    const state = {
      ...
      ranking: { rows: [], byName: new Map(), byKey: new Map() }
    };
    ...
    function teamRpDivisionStrength(value) {
      const division = normalizeDivision(value);
      if (division === "ELITE") return 1.25;
      if (division === "PRO") return 1.15;
      if (division === "LITE") return 1.05;
      if (division === "CORE") return 0.95;
      if (division === "NEO") return 0.85;
      return 1;
    }

    function teamRpRecency(tournament) {
      const timestamp = tournamentTimestamp(tournament);
      if (!timestamp) return 0.45;
      const ageYears = Math.max(0, (Date.now() - timestamp) / 31557600000);
      return Math.max(0.15, Math.exp(-ageYears / 3.5));
    }
    ...
    function teamRpPlayerRanking(player) {
      if (typeof SEH_findPlayerRanking !== "function") return null;
      return SEH_findPlayerRanking(
        state.ranking,
        player.playerKey,
    ...
    function teamRpRosterScore(team) {
      const rows = team.players
        .map((player) => {
          const ranking = teamRpPlayerRanking(player);
          const rp = number(ranking?.ranking_points);
          const rankingRole = cleanText(ranking?.position_group || ranking?.primary_position).toUpperCase();
          const playerRole = cleanText(player.playerType).toLowerCase();
          const isGoalie =
            rankingRole === "G" ||
            playerRole === "goalie" ||
            player.totalGoalieGames > player.totalSkaterGames;
          return { player, rp, isGoalie };
        })
        .filter((row) => row.rp > 0);
      ...
      return {
        raw: selected.reduce((total, row) => total + row.rp, 0),
    ...
    function teamRpHistoryScore(team) {
      let weightedWins = 0;
      let weightedGames = 0;
      let meritRaw = 0;
      let podiums = 0;
      ...
      for (const tournament of team.tournaments) {
        const games = number(tournament.gamesPlayed);
        const wins = number(tournament.wins);
        const factor =
          teamRpRecency(tournament) *
          teamRpDivisionStrength(tournament.division);
        ...
        const placement = number(tournament.divisionRank);
        if (placement >= 1 && placement <= 3) {
          podiums += 1;
          if (placement === 1) firstPlaces += 1;
          meritRaw +=
            (placement === 1 ? 100 : placement === 2 ? 55 : 30) *
            factor;
        }
      }

      const weightedWinRate =
        weightedGames > 0 ? weightedWins / weightedGames : 0;

      return {
        raw: weightedWins * 7 + weightedWinRate * 260,
        meritRaw,
        podiums,
        firstPlaces
      };
    ...
    function calculateTeamRp() {
      const rows = state.teams.map((team) => ({
        team,
        roster: teamRpRosterScore(team),
        history: teamRpHistoryScore(team)
      }));

      const rosterValues = rows.map((row) => row.roster.raw);
      const resultValues = rows.map((row) => row.history.raw);
      const meritValues = rows.map((row) => row.history.meritRaw);

      rows.forEach((row) => {
        const rosterIndex = teamRpPercentile(rosterValues, row.roster.raw);
        const resultIndex = teamRpPercentile(resultValues, row.history.raw);
        const meritIndex = teamRpPercentile(meritValues, row.history.meritRaw);
        const weighted =
          rosterIndex * 0.70 +
          resultIndex * 0.25 +
          meritIndex * 0.05;

        row.team.stats.teamRp =
          row.roster.raw > 0
            ? Math.round(500 + weighted * 4500)
            : 0;
        row.team.stats.teamRpLineup = Math.round(row.roster.raw);
        row.team.stats.teamRpLineupLabel = row.roster.complete
          ? "5+1"
          : `${row.roster.skaters}+${row.roster.goalies}G`;
        row.team.stats.teamRpPodiums = row.history.podiums;
        row.team.stats.teamRpFirstPlaces = row.history.firstPlaces;
        row.team.stats.teamRpRank = null;
      });

      [...state.teams]
        .filter((team) => team.stats.teamRp > 0)
        .sort((a, b) =>
          b.stats.teamRp - a.stats.teamRp ||
          b.stats.teamRpLineup - a.stats.teamRpLineup ||
    ...
    function createTeamCard(team, index) {
      ...
      if (teamRpNode) {
        teamRpNode.textContent = team.stats.teamRp
          ? formatNumber(team.stats.teamRp)
          : "–";
      }
      if (teamRpRankNode) {
        teamRpRankNode.textContent = team.stats.teamRpRank
          ? `#${team.stats.teamRpRank} Sverige`
          : "Ej rankad";
      }
    ...
    async function load() {
      ...
      try {
        const [teams, tournaments, playersResult, leagueDisplayNames, rankingResult] = await Promise.all([
          fetchTeams(),
          fetchTournaments(),
          fetchPlayers().then(
          ...
          (typeof SEH_loadPlayerRanking === "function"
            ? SEH_loadPlayerRanking()
            : Promise.resolve({ rows: [], byName: new Map(), byKey: new Map() })
          ).then(
            (lookup) => ({ ok: true, lookup }),
            (error) => ({
              ok: false,
              error,
              lookup: { rows: [], byName: new Map(), byKey: new Map() }
            })
          )
        ]);
        ...
        state.ranking = rankingResult.lookup;
  
        if (!playersResult.ok) {
          console.warn("Spelarstatistiken kunde inte hämtas:", playersResult.error);
        }
        if (!rankingResult.ok) {
          console.warn("Team RP kunde inte läsa spelar-RP:", rankingResult.error);
        }
