...
function SEH_initHistory() {
  ...
  (() => {
    ...
    async function fetchTournaments() {
      return fetchAllPages(
        "v_ehockey_team_tournaments_chronological",
        {
          select: [
            "team_id",
    ...
    function createTeamCard(team, index) {
      ...
      if (team.stats.topPlayer?.playerKey) {
        ...
        topPlayerLink.href = `#/spelare/${encodeURIComponent(topPlayer.playerKey)}?fromTeam=${encodeURIComponent(team.teamId)}`;
...
function SEH_initPlayers() {
  ...
  (() => {
    ...
    function playerCard(player) {
      ...
      const parameters = new URLSearchParams({ id: player.key });
      link.href = `#/spelare/${encodeURIComponent(player.key)}`;
      link.innerHTML = `
...
function SEH_initPlayer() {
  ...
  (() => {
    ...
    const APP_BUILD = "2026-08-11-player-v24-linked-team-names";
    const config = window.EHOCKEY_CONFIG || {};
    const elements = {
    ...
    function normalizedCompetitionCode(row) {
      ...
      if (explicit && explicit !== "SPORTSGAMER") return explicit;
      const label = `${row.seasonLabel || ""} ${row.leagueName || ""}`;
      if (/european championship league|(^|\W)ecl(\W|$)/i.test(label)) return "ECL";
      if (/swedish championship league|(^|\W)scl(\W|$)/i.test(label)) return "SCL";
      if (/(^|\W)sec(\W|$)/i.test(label)) return "SEC";
      if (/(^|\W)lgel(\W|$)/i.test(label)) return "LGEL";
      if (/sm\s*ehockey|(^|\W)sm(\W|$)/i.test(label)) return "SM";
      return explicit || "SPORTSGAMER";
    }
    ...
    function competitionLine(rows) {
      ...
      rows.forEach((row) => {
        ...
        if (!code || code === "SPORTSGAMER") return;
    ...
    function render(rows) {
      ...
      elements.careerPoints.textContent = formatInteger(points, "0");
      elements.careerGoals.textContent = formatInteger(goals, "0");
      elements.careerAssists.textContent = formatInteger(assists, "0");
      ...
      addStat(elements.goalieCareerStats, "Räddningsprocent", shotsAgainst ? formatSavePercentage(saves / shotsAgainst) : "–");
      ...
      /*
       * Om raden har ett lokalt team_id finns laget redan i Svensk eHockey-
       * databasen och både säsong + lagnamn ska länka till lagets turneringsvy.
       * Vi använder därför inte team_is_linkable som extra spärr här.
       */
      rows.forEach((row) => {
        ...
        /*
         * Rita loggan direkt från samma globala resolver som lagkorten.
         * Detta är viktigt även när raden saknar lokalt team_id.
         */
        SEH_renderTeamLogo(
          teamLogo,
          [row.resolvedLogoUrl || ""],
        ...
        const values = [
          ...
          formatInteger(row.goals, "0"), formatInteger(row.assists, "0"),
      ...
      elements.historyCount.textContent = `${tournamentCount} turneringar`;
    }
    ...
    function renderStandalone(directoryRow, cacheMissing = false) {
      ...
      elements.careerPoints.textContent =
        formatInteger(totalPoints, "0");
      elements.careerGoals.textContent =
        formatInteger(totalGoals, "0");
      elements.careerAssists.textContent =
      ...
      addStat(elements.goalieCareerStats, "Nollor", "–");
    ...
    async function load() {
      ...
      if (!playerKey) {
        showError(new Error("Länken innehåller ingen giltig spelarnyckel."));
        elements.loadingState.hidden = true;
        elements.reloadButton.disabled = false;
        return;
      }
...
function SEH_initTeam() {
  ...
  (() => {
    ...
    const APP_BUILD = "2026-08-11-hash-spa-v1-history-cache-v18-1-no-v16";
    const config = window.EHOCKEY_CONFIG || {};
    ...
    function normalizeTournament(row) {
      ...
      return {
        ...
        nameUsed: row.name_used_in_tournament || row.current_name || "",
        groupId:
          row.group_id === null ||
    ...
    function playerPageUrl(playerKey) {
      const fromTeam = state.team?.teamId
        ? `?fromTeam=${encodeURIComponent(state.team.teamId)}`
        : "";
      return `#/spelare/${encodeURIComponent(playerKey)}${fromTeam}`;
    }
    ...
    async function fetchTournaments(teamId) {
      const rows = await fetchRpcJson(
        "get_ehockey_team_tournaments_linked_v17",
        { p_team_id: teamId }
      );
  
      return dedupeTeamTournaments(rows.map(normalizeTournament));
    }
    ...
    async function fetchUnlinkedTeamPlayers(team) {
      ...
      const pages = await Promise.all(uniqueNames.map((name) => {
        const params = new URLSearchParams({
          select: "*",
          team_id: "is.null",
        ...
        return fetchJson("v_ehockey_player_tournaments_chronological", params);
      }));
    ...
    async function fetchPlayerCounts(teamId) {
      const params = new URLSearchParams({
      ...
      const rows = await fetchJson(
        "v_ehockey_team_tournament_player_counts_all",
        params
      );
    ...
    function playerHistoryRowBelongsToTeam(
      row,
      tournament,
      team
    ) {
      ...
      if (
        ...
        number(row.team_id) === team.teamId
      ) {
        return true;
      }
  
      const sportsGamerTeamId =
        String(row.team_external_id || "").trim();
  
      if (
        sportsGamerTeamId &&
        team.sportsGamerIds.some(
          (value) =>
            String(value || "").trim() ===
            sportsGamerTeamId
        )
    ...
    async function fetchTournamentPlayers(
      ...
      const cacheKey =
        `${tournament.teamId}:${tournament.leagueId}`;
  
      if (state.playerCache.has(cacheKey)) {
        return state.playerCache.get(cacheKey);
      }
  
      const params = new URLSearchParams({
        select: "*",
        league_id: `eq.${tournament.leagueId}`,
        limit: "1000"
      });
      ...
      const promise = fetchJson(
        ...
        .catch((error) => {
          state.playerCache.delete(cacheKey);
          throw error;
        });
  
      state.playerCache.set(cacheKey, promise);
      return promise;
    }
    ...
    function renderHistoryBadges() {
      ...
      const championships = state.tournaments.filter(
        (row) => row.playoffStatusCode === "CHAMPION"
      ).length;
      const finals = state.tournaments.filter(isFinalAppearance).length;
  
      const badges = [];
      if (championships) {
        badges.push({ icon: "♛", label: `${championships} titlar` });
      }
      if (finals) {
        badges.push({ icon: "🏆", label: `${finals} finaler` });
      }
      if (!badges.length) {
        badges.push({ icon: "★", label: `${state.tournaments.length} säsonger` });
      }
  
      for (const badgeData of badges.slice(0, 3)) {
        const badge = document.createElement("span");
    ...
    function renderTeamHeader() {
      ...
      elements.playersHeading.textContent = `Spelare i ${team.currentName} – all-time`;
    ...
    function renderHonours() {
      ...
      const champions = state.tournaments.filter(
        (row) => row.playoffStatusCode === "CHAMPION"
      ).length;
      const finals = state.tournaments.filter(isFinalAppearance).length;
      const bronze = state.tournaments.filter(
    ...
    function createAllTimePlayerCard(player) {
      ...
      link.href = playerPageUrl(player.playerKey);
    ...
    function renderAllTimeTables() {
      ...
      const skaters = [...state.allTimePlayers]
        .filter((player) => player.totalSkaterGames > 0)
        .sort((a, b) => b.totalPoints - a.totalPoints || b.totalSkaterGames - a.totalSkaterGames)
        .slice(0, 10);
      ...
      skaters.forEach((player, index) => {
        ...
        row.append(
          ...
          createTextCell(formatInteger(player.totalPoints, "0"), "history-highlight-cell"),
          createTextCell(formatInteger(player.totalPenaltyMinutes, "0"))
        );
        elements.allTimeSkaterBody.append(row);
      });
  
      const goalies = [...state.allTimePlayers]
        .filter((player) => player.totalGoalieGames > 0)
        .sort((a, b) => b.totalGoalieGames - a.totalGoalieGames ||
          (b.totalGoalieSavePercentage ?? -1) - (a.totalGoalieSavePercentage ?? -1));
      ...
      goalies.forEach((player, index) => {
        ...
        row.append(
          ...
          createTextCell(formatSavePercentage(player.totalGoalieSavePercentage), "history-highlight-cell"),
          createTextCell(formatDecimal(player.totalGoalieGoalsAgainstAverage, 2)),
    ...
    function createPlayerIdentity(
      ...
      if (player.playerKey) {
        name.href = playerPageUrl(player.playerKey);
...
function SEH_initTeamTournament() {
  ...
  (() => {
    ...
    function playerUrl(playerKey, teamId) {
      return `#/spelare/${encodeURIComponent(playerKey)}?fromTeam=${encodeURIComponent(teamId)}`;
    }
...
function SEH_initTournament() {
  ...
  (() => {
    ...
    function playerPageUrl(player) {
      return `#/spelare/${encodeURIComponent(player.player_key)}`;
    }
  ...
  function headerHtml(route) {
    return `
      <div class="seh-header__inner">
        <a class="seh-brand" href="#/" aria-label="Svensk eHockey – startsida">
          <img src="assets/SeHlogga.png" alt="" width="38" height="46">
          <span>
