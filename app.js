...
function SEH_initHistory() {
  ...
  (() => {
    ...
    const APP_BUILD = "2026-09-11-v13012-competition-team-rp";
    ...
    function teamRpScoreTournament(tournament, context) {
      ...
      const reliability = Math.min(gp / 5, 1);
      const adjustedRp =
        50 * (1 - reliability) +
        rawTeamRp * reliability;
    ...
    function calculateTeamRp() {
      ...
      for (const [scopeKey, rawRows] of rowsByScope.entries()) {
        ...
        const scores = playedRows
          .map((row) => ({
            row,
            score: teamRpScoreTournament(row, context)
          }))
          .filter((entry) => entry.score);

        scores.sort((a, b) =>
          b.score.adjustedRp - a.score.adjustedRp ||
          b.score.rawTeamRp - a.score.rawTeamRp ||
          b.score.resultScore - a.score.resultScore ||
          b.score.gdpg - a.score.gdpg ||
          cleanText(a.row.nameUsed || a.row.currentName).localeCompare(
            cleanText(b.row.nameUsed || b.row.currentName),
            "sv-SE"
          )
        );
        ...
        scores.forEach((entry, index) => {
          ...
          entry.score.teamCount = scores.length;
          byParticipant.set(entry.score.participantKey, entry.score);
        });

        const representative = rows[0] || null;
        scopes.set(scopeKey, {
          key: scopeKey,
          label: representative ? seasonFilterLabel(representative) : scopeKey,
          pointsModel: context.pointsModel?.id || "win-rate",
      ...
      state.teamRpScopes = scopes;

      for (const team of state.teams) {
        team.stats.teamRpByScope = new Map();

        for (const tournament of team.tournaments) {
          const scopeKey = seasonFilterKey(tournament);
          const scope = scopes.get(scopeKey);
          const participantKey = teamRpParticipantKey(tournament);
          const score = scope?.byParticipant.get(participantKey) || null;
          if (score) team.stats.teamRpByScope.set(scopeKey, score);
        }

        team.stats.teamRp = 0;
        team.stats.teamRpRank = null;
        team.stats.teamRpGames = 0;
        team.stats.teamRpScopeTeamCount = 0;
        team.stats.teamRpDetails = null;
      }
    }

    function applyTeamRpScope(scopeKey) {
      const activeScope =
        scopeKey && scopeKey !== "all"
          ? scopeKey
          : "";

      state.activeTeamRpScope = activeScope;

      for (const team of state.teams) {
        const score = activeScope
          ? team.stats.teamRpByScope?.get(activeScope) || null
          : null;

        team.stats.teamRp = score?.teamRp || 0;
        team.stats.teamRpRank = score?.rank || null;
        team.stats.teamRpGames = score?.gp || 0;
        team.stats.teamRpScopeTeamCount = score?.teamCount || 0;
        team.stats.teamRpDetails = score;
      }
    }
    ...
    function createTeamCard(team, index) {
      ...
      if (teamRpNode) {
        teamRpNode.textContent = team.stats.teamRpDetails
          ? formatNumber(team.stats.teamRp)
          : "–";
      }
      if (teamRpRankNode) {
        if (team.stats.teamRpDetails) {
          teamRpRankNode.textContent =
            `#${team.stats.teamRpRank} av ${team.stats.teamRpScopeTeamCount} · ${team.stats.teamRpGames} GP`;
        } else {
          teamRpRankNode.textContent = state.activeTeamRpScope
            ? "Inga matcher"
            : "Välj säsong";
        }
      }
    ...
    function compareTeams(a, b, sortMode) {
      ...
      if (sortMode === "team-rp") {
        return b.stats.teamRp - a.stats.teamRp ||
