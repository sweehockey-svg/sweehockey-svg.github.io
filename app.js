...
function SEH_initHistory(options = {}) {
  ...
  return (() => {
    ...
    function teamRpPercentileScore(values, value) {
      const sorted = values
      ...
      return teamRpClamp((midpointRank / (sorted.length - 1)) * 100, 0, 100);
    }

    function teamRpParticipantKey(tournament) {
      const teamId = number(tournament.teamId);
      if (teamId > 0) return `team:${teamId}`;

      const sportsGamerId = cleanText(tournament.sportsGamerTeamId);
      if (sportsGamerId) return `sg:${sportsGamerId}`;

      const nameKey = normalizedTeamNameKey(
        tournament.nameUsed || tournament.currentName
      );
    ...
    function teamRpDedupeScopeRows(rows) {
      ...
      for (const row of rows) {
        const participantKey = teamRpParticipantKey(row);
        if (!participantKey) continue;

        const current = byParticipant.get(participantKey);
        if (!current) {
        ...
        const currentScore =
          (current.hasStatistics ? 1000000 : 0) +
          number(current.gamesPlayed) * 100 +
          number(current.playoffGames);
        const rowScore =
          (row.hasStatistics ? 1000000 : 0) +
          number(row.gamesPlayed) * 100 +
          number(row.playoffGames);

        if (rowScore > currentScore) byParticipant.set(participantKey, row);
      }

      return [...byParticipant.values()];
    }

    function teamRpDraws(tournament) {
      return Math.max(
        0,
        number(tournament.gamesPlayed) -
          number(tournament.wins) -
          number(tournament.losses)
      );
    }

    function teamRpDetectPointsModel(rows) {
      const playedRows = rows.filter((row) => number(row.gamesPlayed) > 0);
      if (
        !playedRows.length ||
        !playedRows.some((row) => number(row.tablePoints) > 0)
      ) {
        return null;
      }
      ...
      const candidates = [
        {
          ...
          maxPointsPerGame: 2,
          expected: (row) =>
            number(row.wins) * 2 + number(row.overtimeLosses)
        },
        {
          id: "3pt-ot",
          maxPointsPerGame: 3,
          expected: (row) => {
            const overtimeWins = Math.min(
              number(row.wins),
              number(row.overtimeWins)
            );
            const regulationWins = Math.max(
              0,
              number(row.wins) - overtimeWins
            );
            return (
              regulationWins * 3 +
              overtimeWins * 2 +
              number(row.overtimeLosses)
            );
          }
        },
        {
          ...
          maxPointsPerGame: 3,
          expected: (row) =>
            number(row.wins) * 3 + teamRpDraws(row)
        },
        {
          id: "2pt-draw",
          maxPointsPerGame: 2,
          expected: (row) =>
            number(row.wins) * 2 + teamRpDraws(row)
        },
        {
          id: "2pt-win",
          maxPointsPerGame: 2,
          expected: (row) => number(row.wins) * 2
        },
        {
          id: "3pt-win",
          maxPointsPerGame: 3,
          expected: (row) => number(row.wins) * 3
        }
      ];
      ...
      return (
        candidates.find((candidate) =>
          playedRows.every(
            (row) =>
              Math.abs(
                number(row.tablePoints) - candidate.expected(row)
              ) < 0.001
          )
        ) || null
      );
    ...
    function teamRpScoreTournament(tournament, context) {
      const gp = number(tournament.gamesPlayed);
      if (gp <= 0) return null;

      const wins = teamRpClamp(number(tournament.wins), 0, gp);
      const losses = teamRpClamp(number(tournament.losses), 0, gp);
      const draws = teamRpDraws(tournament);
      const goalsFor = Math.max(0, number(tournament.goalsFor));
      const goalsAgainst = Math.max(0, number(tournament.goalsAgainst));
      ...
      const pointsPercentage = context.pointsModel
        ? teamRpClamp(
            number(tournament.tablePoints) /
              (gp * context.pointsModel.maxPointsPerGame),
            0,
            1
          )
        : null;
      ...
      const offenseScore = teamRpPercentileScore(
        context.gfpgValues,
        gfpg
      );
      ...
      return {
        participantKey: teamRpParticipantKey(tournament),
        gp,
        wins,
        draws,
        losses,
        ...
        winRate,
        pointsPercentage,
        pointsModel: context.pointsModel?.id || "win-rate",
        resultScore,
        goalDifferenceScore,
        offenseScore,
        defenseScore,
    ...
    function teamRpRecencyWeight(tournament, latestTimestamp) {
      const timestamp = tournamentTimestamp(tournament);
      if (!timestamp || !latestTimestamp) return 0.35;

      const ageYears = Math.max(
        0,
        (latestTimestamp - timestamp) / 31557600000
      );

      return Math.max(0.18, Math.exp(-ageYears / 3.25));
    }

    function calculateTeamRp() {
      const rowsByScope = new Map();

      for (const tournament of state.tournaments) {
        const scopeKey = seasonFilterKey(tournament);
        if (!rowsByScope.has(scopeKey)) rowsByScope.set(scopeKey, []);
        rowsByScope.get(scopeKey).push(tournament);
      }

      const scopes = new Map();

      for (const [scopeKey, rawRows] of rowsByScope.entries()) {
        const rows = teamRpDedupeScopeRows(rawRows);
        const playedRows = rows.filter((row) => number(row.gamesPlayed) > 0);
        const context = {
          pointsModel: teamRpDetectPointsModel(playedRows),
          gfpgValues: playedRows.map(
            (row) => number(row.goalsFor) / number(row.gamesPlayed)
          ),
          gapgValues: playedRows.map(
            (row) => number(row.goalsAgainst) / number(row.gamesPlayed)
          )
        };

        const byParticipant = new Map();
        for (const row of playedRows) {
          const score = teamRpScoreTournament(row, context);
          if (score) byParticipant.set(score.participantKey, score);
        }
      ...
      const latestTimestamp = Math.max(
        0,
        ...state.tournaments
          .filter((row) => number(row.gamesPlayed) > 0)
      ...
      for (const team of state.teams) {
        ...
        for (const tournament of team.tournaments) {
          const scope = scopes.get(seasonFilterKey(tournament));
          const score = scope?.byParticipant.get(
            teamRpParticipantKey(tournament)
          );
          if (!score) continue;

          const recencyWeight = teamRpRecencyWeight(
            tournament,
            latestTimestamp
          );
          const sampleWeight = Math.min(score.gp, 20);
          const weight = sampleWeight * recencyWeight;
          const tournamentCompleteness = Math.min(score.gp / 20, 1);

          weightedRp += score.rawTeamRp * weight;
          totalRp += score.rawTeamRp * recencyWeight * tournamentCompleteness;
          weightedResult += score.resultScore * weight;
          weightedGoalDifference += score.goalDifferenceScore * weight;
          weightedOffense += score.offenseScore * weight;
          weightedDefense += score.defenseScore * weight;
          totalWeight += weight;
          effectiveGp += score.gp * recencyWeight;
        ...
        team.stats.teamRpAverage = Math.round(teamRpClamp(adjustedRp, 0, 100));
        team.stats.teamRpTotal = Math.round(Math.max(0, totalRp));
        team.stats.teamRp = team.stats.teamRpTotal;
        team.stats.teamRpRank = null;
        team.stats.teamRpGames = gp;
        team.stats.teamRpScopeTeamCount = 0;
        team.stats.teamRpDetails = {
          gp,
          wins,
          draws,
          losses,
          ...
          adjustedRp,
          totalRp,
          averageRp: team.stats.teamRpAverage,
          resultScore: totalWeight > 0 ? weightedResult / totalWeight : 50,
          goalDifferenceScore:
            totalWeight > 0 ? weightedGoalDifference / totalWeight : 50,
          offenseScore: totalWeight > 0 ? weightedOffense / totalWeight : 50,
          defenseScore: totalWeight > 0 ? weightedDefense / totalWeight : 50
        };
      }

      const ranked = state.teams
        .filter((team) => team.stats.teamRpGames > 0)
        .sort((a, b) =>
          b.stats.teamRpTotal - a.stats.teamRpTotal ||
          b.stats.teamRpAverage - a.stats.teamRpAverage ||
          b.stats.teamRpDetails.rawTeamRp - a.stats.teamRpDetails.rawTeamRp ||
          b.stats.teamRpDetails.effectiveGp - a.stats.teamRpDetails.effectiveGp ||
          a.currentName.localeCompare(b.currentName, "sv-SE")
        );

      ranked.forEach((team, index) => {
        team.stats.teamRpRank = index + 1;
      });

      for (const team of state.teams) {
        team.stats.teamRpScopeTeamCount = ranked.length;
      }
    }
    ...
    function createTeamCard(team, index) {
      ...
      const teamRpNode = fragment.querySelector(".metric-team-rp");
      const teamRpRankNode = fragment.querySelector(".metric-team-rank");
      if (teamRpNode) {
        teamRpNode.textContent = team.stats.teamRpGames > 0
          ? formatNumber(team.stats.teamRpTotal)
          : "0";
      }
      if (teamRpRankNode) {
        teamRpRankNode.textContent = team.stats.teamRpGames > 0
          ? `#${team.stats.teamRpRank} av ${team.stats.teamRpScopeTeamCount} · SNITT ${formatNumber(team.stats.teamRpAverage)} · ${team.stats.teamRpGames} GP`
          : "SNITT 50 · Nytt lag";
      }
    ...
    function compareTeams(a, b, sortMode) {
      ...
      if (sortMode === "team-rp") {
        const ratedDifference =
          Number(b.stats.teamRpGames > 0) -
          Number(a.stats.teamRpGames > 0);
        return ratedDifference ||
          b.stats.teamRpTotal - a.stats.teamRpTotal ||
          b.stats.teamRpAverage - a.stats.teamRpAverage ||
          b.stats.teamRpGames - a.stats.teamRpGames ||
          nameA.localeCompare(nameB, "sv-SE");
      }
      if (sortMode === "team-rp-average") {
        const ratedDifference =
          Number(b.stats.teamRpGames > 0) -
          Number(a.stats.teamRpGames > 0);
        return ratedDifference ||
      ...
      for (const tournament of visibleTournaments) {
        ...
        nameCell.append(
          createInternalLink(
            ...
            `#/lag/${encodeURIComponent(state.team.teamId)}`,
            "history-table-link"
          )
        );
...
(() => {
  ...
  const APP_BUILD = "2026-09-12-v13024-standalone-fantasy";
    ...
    `<div class="home-stage__cta-wrap">
      <div class="home-stage__cta-row" aria-label="Svensk eHockey i mobilen">
        <a class="home-app-cta home-app-cta--android" href="/android.html" aria-label="Ladda ner Svensk eHockey för Android">
          ...
          <span class="home-app-cta__copy">
            <small>ANDROID</small>
            <strong>Ladda ner appen</strong>
            <em>APK · Android 7.0+</em>
          </span>
          <span class="home-app-cta__arrow" aria-hidden="true">→</span>
        </a>

        <a class="home-app-cta home-app-cta--iphone" href="/iphone.html" aria-label="Lägg till Svensk eHockey som webbapp på iPhone">
          <span class="home-app-cta__icon" aria-hidden="true">
            <svg viewBox="0 0 24 24" focusable="false" aria-hidden="true">
              <rect x="6.5" y="2.5" width="11" height="19" rx="2.6" fill="none" stroke="currentColor" stroke-width="1.8"/>
              <path d="M10 5h4M10.3 18.6h3.4" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/>
              <path d="M12 14V8m0 0-2.2 2.2M12 8l2.2 2.2" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
          </span>
          <span class="home-app-cta__copy">
            <small>iPHONE · WEBBAPP</small>
            <strong>Lägg till på hemskärmen</strong>
            <em>Ingen App Store behövs</em>
          </span>
          <span class="home-app-cta__arrow" aria-hidden="true">→</span>
        </a>
      </div>
  ...
  templates.records = `
    <main class="directory-shell seh-recordbook-shell" id="sehRecordbookRoot">
      ...
      <section class="seh-recordbook-panel">
        ...
        <div class="seh-record-podium" data-record-podium aria-label="Topp 3"></div>
...
(() => {
  ...
  const adminSection = window.SEH_ROUTE?.params?.adminSection || "home";
  const isPlayerAdminPage = adminSection === "players";
  if (isPlayerAdminPage) {
    const hero = document.querySelector("#spaRouteView[data-route=\"admin\"] .admin-hero");
    const back = document.querySelector("#spaRouteView[data-route=\"admin\"] .writer-back");
    if (hero) hero.innerHTML = '<p class="directory-kicker">SVENSK eHOCKEY / ADMIN / SPELARE</p><h1>Spelarhantering</h1><p>Discord-kopplingar, spelarprofiler, felrapporter och Free Agents samlat på en egen adminsida.</p>';
    if (back) { back.href = "#/admin"; back.textContent = "← Till Admincenter"; }
  }
  const cfg = window.SEH_CONFIG || window.EHOCKEY_CONFIG || window.APP_CONFIG || window.config || {};
  const supabaseUrl = cfg.supabaseUrl || cfg.SUPABASE_URL || '';
  const supabaseKey = cfg.supabasePublishableKey || cfg.supabaseAnonKey || cfg.SUPABASE_ANON_KEY || cfg.SUPABASE_PUBLISHABLE_KEY || '';
  ...
  const playerSyncStartedKey='seh_player_sync_started_at',statsSyncStartedKey='seh_player_stats_sync_started_at',syncPollMs=15000,syncPollMaxMs=50*60*1000;
  const syncPollingIsFresh=(key)=>{const startedAt=Number(sessionStorage.getItem(key)||0);return startedAt>0&&Date.now()-startedAt<=syncPollMaxMs};
  const stopSyncPolling=(key)=>sessionStorage.removeItem(key);
  ...
  function faNotifyStatus(message,tone=''){
    const el=$('faAdminNotifyState');
    if(!el)return;
    el.textContent=message||'';
    el.dataset.tone=tone||'';
  }
  function describeNotifyResult(result){
    if(result?.sent>0)return {text:`Discord: ${result.sent} notis${result.sent>1?'er':''} skickad${result.sent>1?'e':''}.`,tone:'success'};
    if(result?.failed>0){
      const first=result?.errors?.[0]?.error;
      return {text:`Discord: ${result.failed} notis${result.failed>1?'er':''} kunde inte skickas${first?` – ${first}`:''}`,tone:'error'};
    }
    if(result?.configured===false)return {text:'Discord: bot-token saknas i Supabase.',tone:'error'};
    if(result?.error)return {text:`Discord: ${result.error}`,tone:'error'};
    return {text:'Discord: inga väntande notiser.',tone:'success'};
  }
  ...
  function faRenderApprovals(){
    ...
    if(linkHost){
      ...
      for(const row of faLinkRequests){const item=document.createElement('article');item.className='fa-admin-request-row';item.dataset.adminRequestKey=`link:${row.user_id}`;item.innerHTML=`<div><span>DISCORD</span><strong>${escapeHtml(row.discord_username||row.discord_user_id||'Discord-konto')}</strong><small>vill kopplas till <b>${escapeHtml(faApprovalPlayerName(row.requested_player_key))}</b></small></div><div class="fa-admin-request-actions"><button type="button" data-fa-link-approve="${escapeHtml(row.user_id)}">Godkänn</button><button type="button" class="writer-secondary" data-fa-link-reject="${escapeHtml(row.user_id)}">Avslå</button></div>`;linkHost.append(item);}
    }
    if(approvedHost){
    ...
    if(requestHost){
      ...
      for(const row of faApprovalRequests){const typeLabel=row.request_type==='remove'?'TA BORT':row.request_type==='update'?'ÄNDRING':'NY FA';const detail=[row.positions_text,row.levels_text,row.message].filter(Boolean).join(' · ');const item=document.createElement('article');item.className=`fa-admin-request-row${row.request_type==='remove'?' is-remove':''}`;item.dataset.adminRequestKey=`fa:${row.id}`;item.innerHTML=`<div><span>${escapeHtml(typeLabel)} · ${escapeHtml(row.discord_username||'Discord')}</span><strong>${escapeHtml(faApprovalPlayerName(row.player_key))}</strong><small>${escapeHtml(detail||'Ingen extra kommentar')}</small></div><div class="fa-admin-request-actions"><button type="button" data-fa-request-approve="${row.id}">${row.request_type==='remove'?'Godkänn borttagning':'Godkänn'}</button><button type="button" class="writer-secondary" data-fa-request-reject="${row.id}">Avslå</button></div>`;requestHost.append(item);}
    }
  }
  ...
  function playerAdminStampRefresh(){
    playerAdminLastRefreshAt=Date.now();
    const stamp=$('adminPlayerLiveUpdated');
    if(stamp)stamp.textContent=`Senast ${new Date(playerAdminLastRefreshAt).toLocaleTimeString('sv-SE',{hour:'2-digit',minute:'2-digit',second:'2-digit'})}`;
  }
  function playerAdminHighlightNew(keys){
    if(!keys?.length)return;
    requestAnimationFrame(()=>{
      for(const key of keys){
        const safe=window.CSS?.escape?CSS.escape(key):key.replace(/([:\[\]#.;,])/g,'\$1');
        const node=document.querySelector(`[data-admin-request-key="${safe}"]`);
  ...
  async function refreshPlayerAdminQueues({announce=true}={}){
    ...
    try{
      const previous=new Set(playerAdminLastPendingKeys);
      await loadFreeAgentApprovals();
      const current=playerAdminPendingKeys();
      const newKeys=playerAdminHasSnapshot?[...current].filter((key)=>!previous.has(key)):[];
      playerAdminLastPendingKeys=current;
      playerAdminHasSnapshot=true;
      updatePlayerAdminCounters();
      playerAdminStampRefresh();
      if(announce&&newKeys.length){
        playerAdminSetLiveState(`${newKeys.length} nytt${newKeys.length===1?'':'a'} ärende${newKeys.length===1?'':'n'} kom in.`, 'new');
        playerAdminHighlightNew(newKeys);
        window.clearTimeout(playerAdminNewStateTimer);
        playerAdminNewStateTimer=window.setTimeout(()=>playerAdminSetLiveState('Nya ärenden hämtas automatiskt var 15:e sekund.',''),7000);
      }else if(!newKeys.length){
        playerAdminSetLiveState('Nya ärenden hämtas automatiskt var 15:e sekund.','');
      }
    }catch(error){
      console.warn('Automatisk ärendekoll misslyckades',error);
      playerAdminSetLiveState(`Kunde inte uppdatera: ${error?.message||error}`,'error');
    }finally{
      playerAdminRefreshInFlight=false;
      if(button)button.disabled=false;
    }
  }
  function stopPlayerAdminAutoRefresh(){
    window.clearTimeout(playerAdminAutoTimer);playerAdminAutoTimer=0;
    window.clearTimeout(playerAdminNewStateTimer);playerAdminNewStateTimer=0;
    window.removeEventListener('focus',playerAdminFocusHandler);
    document.removeEventListener('visibilitychange',playerAdminVisibilityHandler);
    if(window.SEH_adminPlayerAutoRefreshCleanup===stopPlayerAdminAutoRefresh)window.SEH_adminPlayerAutoRefreshCleanup=null;
  }
  function schedulePlayerAdminAutoRefresh(delay=15000){
    window.clearTimeout(playerAdminAutoTimer);
    playerAdminAutoTimer=window.setTimeout(async()=>{
      if(!document.hidden)await refreshPlayerAdminQueues({announce:true});
      schedulePlayerAdminAutoRefresh(15000);
    },delay);
  }
  function playerAdminFocusHandler(){
    if(!isPlayerAdminPage||document.hidden)return;
    if(Date.now()-playerAdminLastRefreshAt>5000)refreshPlayerAdminQueues({announce:true});
  }
  function playerAdminVisibilityHandler(){
    if(document.hidden)return;
    if(Date.now()-playerAdminLastRefreshAt>5000)refreshPlayerAdminQueues({announce:true});
  }
  function startPlayerAdminAutoRefresh(){
    if(!isPlayerAdminPage||writer?.role!=='admin')return;
    stopPlayerAdminAutoRefresh();
    playerAdminLastPendingKeys=playerAdminPendingKeys();
    playerAdminHasSnapshot=true;
    updatePlayerAdminCounters();
    playerAdminStampRefresh();
    playerAdminSetLiveState('Nya ärenden hämtas automatiskt var 15:e sekund.','');
    window.addEventListener('focus',playerAdminFocusHandler);
    document.addEventListener('visibilitychange',playerAdminVisibilityHandler);
    schedulePlayerAdminAutoRefresh(15000);
    window.SEH_adminPlayerAutoRefreshCleanup=stopPlayerAdminAutoRefresh;
  }
...
Free Agent-annonsen ligger kvar, men Discord-kontot måste kopplas och godkännas igen för att spelaren ska kunna hantera den själv.`))return;
  ...
  async function loadFreeAgentAdmin(){
    ...
    try{
      ...
      playerAdminLastPendingKeys=playerAdminPendingKeys();playerAdminHasSnapshot=true;updatePlayerAdminCounters();playerAdminStampRefresh();
      const notify=await flushDiscordNotifications();
      const notifyState=describeNotifyResult(notify);
      faNotifyStatus(notifyState.text,notifyState.tone);
      faSetStatus('');
  ...
  function bindFreeAgentAdmin(){
    ...
    $('adminPlayerRefreshNow')?.addEventListener('click',()=>refreshPlayerAdminQueues({announce:false}));
    $('faAdminNotifyRetry')?.addEventListener('click',async()=>{
      const button=$('faAdminNotifyRetry');
      if(button)button.disabled=true;
      faNotifyStatus('Discord: skickar väntande notiser…','working');
      try{
        const result=await flushDiscordNotifications();
        const state=describeNotifyResult(result);
        faNotifyStatus(state.text,state.tone);
      }finally{
        if(button)button.disabled=false;
      }
    });
    ...
    authForm?.addEventListener('submit', async (event) => {
      ...
      try {
        ...
        if (row?.role === 'admin') {
          ...
          if (requestId) refresh(syncPollingIsFresh(playerSyncStartedKey));
          if (statsRequestId) refreshStats(syncPollingIsFresh(statsSyncStartedKey));
        }
      } catch (err) {
        try { await sb.auth.signOut(); } catch (_) {}
        updateStandaloneHeader(null, null);
  ...
  async function refresh(poll=false){ if(!requestId) return; clearTimeout(timer);busy(true); try { const d=await invoke('status'); const done=d.state==='completed'; setStatus(done ? (d.conclusion==='success'?'Klart – spelarregistret är uppdaterat.':'Synkningen misslyckades.') : 'Synkningen körs…', done&&d.conclusion==='success'?'success':done?'error':'working'); if(d.run_url){const a=document.createElement('a');a.href=d.run_url;a.target='_blank';a.rel='noopener';a.textContent=' Visa körlogg';$('playerSyncStatus').append(a)} if(poll&&!done){if(syncPollingIsFresh(playerSyncStartedKey))timer=setTimeout(()=>refresh(true),syncPollMs);else setStatus('Statuskontrollen stoppades efter 50 minuter. Tryck Kontrollera status för en manuell kontroll.','error')}if(done)stopSyncPolling(playerSyncStartedKey); } catch(e){stopSyncPolling(playerSyncStartedKey);setStatus('Fel: '+(e.message||e),'error')} finally{busy(false)} }
  ...
  $('startPlayerSync').onclick=async()=>{if(!confirm('Starta synkningen av svenska SportsGamer-spelare nu?'))return; requestId=makeId();sessionStorage.setItem('seh_player_sync_request_id',requestId);sessionStorage.setItem(playerSyncStartedKey,String(Date.now()));busy(true);setStatus('Startar synkningen…','working');try{await invoke('start');await refresh(true)}catch(e){stopSyncPolling(playerSyncStartedKey);setStatus('Fel: '+(e.message||e),'error');busy(false)}};
  $('refreshPlayerSync').onclick=()=>refresh(false);
  $('startStatsSync').onclick=async()=>{if(!confirm('Hämta ny statistik för alla registrerade svenska SportsGamer-spelare nu? SportsGamer-databasen kommer endast att läsas.'))return;statsRequestId=makeId();sessionStorage.setItem('seh_player_stats_sync_request_id',statsRequestId);sessionStorage.setItem(statsSyncStartedKey,String(Date.now()));busyStats(true);setStatsStatus('Startar statistiksynkningen…','working');try{await invokeStats('start');await refreshStats(true)}catch(e){stopSyncPolling(statsSyncStartedKey);setStatsStatus('Fel: '+(e.message||e),'error');busyStats(false)}};
  ...
  function parseRoute() {
    ...
    if (parts[0] === "admin" && parts[1] === "spelare" && parts.length === 2) {
      return {
        ...route,
        key: "admin",
        label: "Spelarhantering",
        active: "admin",
        params: { adminSection: "players" }
      };
    }
  ...
  function titleFor(route) {
    if (route.key === "admin" && route.params?.adminSection === "players") {
      return "Spelarhantering – Svensk eHockey";
    }
    if (route.key === "season") {
  ...
  async function render() {
    ...
    if (route.key === "admin" && route.params?.adminSection === "players") {
      document.body.classList.add("admin-player-page");
    }
